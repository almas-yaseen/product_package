...
async function rotateAndSave() {
    ...
    for(let i = 0; i < max; i+=inc) {
        ...
        imgData.push(renderer.domElement.toDataURL("/static/image/scene.gltf") );
    }
...
function saveAsImage(imgData,index) {
    ...
    try {
        var strMime = "/static/images/scene.gltf";
        saveFile(imgData.replace(strMime, "/static/images/scene.gltf"),`${textParams.text}${index+1}.jpg`);
    }
    catch (e) {
        console.log(e);
        return;
